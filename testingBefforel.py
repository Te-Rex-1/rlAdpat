...
# Logging function to record vehicle queue lengths
...
if __name__ == '__main__':
    ...
    for e in range(episodes):
        ...
        while traci.simulation.getMinExpectedNumber() > 0 and stepz < 7000:
            # Record queue before action selection
            record_vehicle_queue(stepz, "queue_before_action.txt")
            traci.simulationStep()
            state = sumoInt.getState()
            action = agent.act(state)
            light = state[2]

            # Depending on the action and current light phase, execute control logic.
            # Logging is performed at the beginning and end of each branch.
            if action == 0 and light[0][0][0] == 0:
                ...
                for i in range(10):
                    ...
                    waiting_time += (traci.edge.getLastStepHaltingNumber('1si') +
                                     ...
                                     traci.edge.getLastStepHaltingNumber('4si'))
                    traci.simulationStep()
                record_vehicle_queue(stepz, "queue_after_action.txt")
        ...
        # At the end of an episode, mark the last transition as terminal.
        mem = agent.memory[-1]
        del agent.memory[-1]
